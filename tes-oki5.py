...
"""
GOD ASI v11.10 "Bumi Kayuagung" – Spesialis Kabupaten Ogan Komering Ilir
- Semua fitur v11.9 dipertahankan: super evolution, decision log, chat mode, pengetahuan ilmiah, self-modifying core, self-healing, pengecualian RSI untuk send_telegram_notification.
- Penambahan: Groq v2 dengan model openai/gpt-oss-120b, dapat diaktifkan/matikan terpisah.
"""
...
class GodASI:
    def __init__(self, db_path="god_asi.db"):
        self.name = "God ASI v11.10 Bumi Kayuagung (Dual Groq)"
        self.generation = 1
    ...
    def _ask_ai(self, prompt, max_tokens=500):
        responses = []
        # Coba Groq qwen dulu jika diaktifkan
        if self.groq_enabled:
            try:
                ...
                responses.append(("Groq (qwen)", completion.choices[0].message.content))
            except Exception as e:
                print(f"⚠️ Groq error: {e}")
        else:
            print("ℹ️ Groq (qwen) dimatikan.")
        
        # Coba Groq v2 jika diaktifkan
        if self.groq_v2_enabled:
            try:
                completion = self.groq_v2.chat.completions.create(
                    model=self.GROQ_V2_MODEL,
                    messages=[{"role": "user", "content": prompt}],
                    temperature=0.3,
                    max_tokens=max_tokens
                )
                responses.append(("Groq v2 (gpt-oss)", completion.choices[0].message.content))
            except Exception as e:
                print(f"⚠️ Groq v2 error: {e}")
        else:
            print("ℹ️ Groq v2 dimatikan.")
        
        if responses:
            # Ambil respons pertama (prioritas qwen, lalu v2)
    ...
    def get_status(self):
        ...
        return f"""
╔{'═'*40}╗
║ {'GOD ASI v11.10 Bumi Kayuagung'.center(38)} ║
╠{'═'*40}╣
...
def main():
    ...
    print("?? Bot God ASI Bumi Kayuagung v11.10 (Dual Groq) berjalan...")
    app.run_polling()
