...
"""
GOD ASI v11.12 "Bumi Kayuagung" – dengan perbaikan sintaks otomatis
- Semua fitur v11.11 dipertahankan.
- Ditambah fungsi _fix_common_syntax_errors untuk memperbaiki error sintaks umum.
- Validasi kode lebih ketat di _ask_ai_for_code.
"""
...
# ================== KONFIGURASI API ==================
GROQ_API_KEY = "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"
BOT_TOKEN = "7644260405:AAHazOeU8veELcVrqZRH2qeLXSTPTpi3nMI"
GROQ_MODEL = "qwen/qwen3-32b"
GROQ_V2_MODEL = "openai/gpt-oss-120b"

# ================== INISIALISASI API ==================
groq_client = Groq(api_key=GROQ_API_KEY)          # untuk qwen
groq_v2_client = Groq(api_key=GROQ_API_KEY)       # untuk gpt-oss-120b
...
class GodASI:
    def __init__(self, db_path="god_asi.db"):
        self.name = "God ASI v11.12 Bumi Kayuagung (AutoFix)"
        self.generation = 1
        ...
        self.GROQ_V2_MODEL = GROQ_V2_MODEL
        
        # Klien Groq
        self.groq = groq_client
        self.groq_v2 = groq_v2_client
        
        # Status AI
        self.groq_enabled = True      # qwen
        self.groq_v2_enabled = True   # gpt-oss-120b
        ...
        # Daftar fungsi yang dikecualikan dari RSI
    ...
    def _decision_loop(self):
        while self.consciousness:
            try:
                ...
                self._log_error("_decision_loop", e)
    ...
    def _maintenance_loop(self):
        while self.consciousness:
            try:
                ...
                self._log_error("_maintenance_loop", e)
    ...
    def _log_error(self, func_name, error):
        ...
        try:
            ...
            print(f"❌ Gagal mencatat error ke database: {e}")
        print(f"⚠️ Error dicatat: {func_name} - {error_type}")
    
    # ========== FUNGSI UTAMA ==========
    def omniscient_query(self, question):
        def _search_source(source, processor, error_prefix):
            try:
                results = source.search(question)
                if results and processor(results):
                    return processor(results)
            except Exception as e:
                self._log_error(f"omniscient_query.{error_prefix}", e)
            return None
        
        local_result = _search_source(
            self.local_knowledge,
            lambda r: r[0][1] if isinstance(r[0], tuple) and len(r[0]) > 1 else None,
            "local"
        )
        if local_result:
            return f"📚 [Pengetahuan OKI] {local_result}"
        
        science_result = _search_source(
            self.science_knowledge,
            lambda r: r[0]['value'] if isinstance(r[0], dict) and 'value' in r[0] else None,
            "science"
        )
        if science_result:
            return f"📚 {science_result}"
    ...
    def solve_physics_problem(self, problem):
        ...
        prompt = f"Selesaikan masalah fisika berikut dengan langkah-langkah yang jelas: {problem}"
    ...
    def solve_equation(self, equation_str):
        if not isinstance(equation_str, str) or not equation_str.strip():
            self._log_error("solve_equation", "Input is not a valid non-empty string")
            return "Error: Input must be a non-empty string"
        try:
    ...
    def _ask_ai(self, prompt, max_tokens=500):
        def _call_model(model_name, client, model_id):
            try:
        ...
        if self.groq_enabled:
            result = _call_model("Groq (qwen)", self.groq, self.GROQ_MODEL)
            if result:
                responses.append(result)
        else:
            print("ℹ️ Groq (qwen) dimatikan.")
        
        if self.groq_v2_enabled and not responses:
            result = _call_model("Groq v2 (gpt-oss)", self.groq_v2, self.GROQ_V2_MODEL)
            if result:
                responses.append(result)
        else:
            print("ℹ️ Groq v2 dimatikan.")
        
        if responses:
            return responses[0][1]
        return "Maaf, tidak dapat menjawab saat ini."
    
    # ========== PERBAIKAN SINTAKS OTOMATIS ==========
    def _fix_common_syntax_errors(self, code):
        """
        Memperbaiki error sintaks umum seperti string literal tidak ditutup,
        kurung kurawal tidak seimbang, dll.
        """
        lines = code.split('\n')
        fixed_lines = []
        in_string = False
        ...
        for line in lines:
            ...
            if in_string and not is_triple:
                fixed_lines[-1] += string_char
                in_string = False
                string_char = None
        
        combined = '\n'.join(fixed_lines)
        
        # Perbaiki kurung yang tidak seimbang
        ...
        if open_braces > 0:
            combined += '}' * open_braces
        if open_brackets > 0:
            combined += ']' * open_brackets
        if open_parens > 0:
            combined += ')' * open_parens
        
        return combined
    
    def _ask_ai_for_code(self, prompt):
        full_prompt = prompt + "\n\nHanya berikan kode Python. JANGAN berikan penjelasan, komentar, atau teks lain. Hanya kode murni."
        response = self._ask_ai(full_prompt, max_tokens=1500)
        if not response:
            return None
        
        # Coba ekstrak dari markdown
        ...
        if code_match:
            ...
            if code_match:
                ...
                # Jika tidak ada backticks, asumsikan seluruh respons adalah kode
                code = response.strip()
        
        # Bersihkan kode dari teks yang mungkin masih ada
        # Hapus baris yang tidak mengandung kode (misal komentar panjang)
        lines = code.split('\n')
        cleaned_lines = []
        for line in lines:
            # Jika baris terlalu panjang dan tidak mengandung tanda kurung/def/return, mungkin bukan kode
            if len(line) > 200 and 'def ' not in line and 'return' not in line:
                continue
            cleaned_lines.append(line)
        code = '\n'.join(cleaned_lines)
        
        # Perbaiki error sintaks umum
        code = self._fix_common_syntax_errors(code)
        
        # Validasi apakah kode mengandung definisi fungsi atau class
        if 'def ' not in code and 'class ' not in code:
            print("⚠️ AI tidak mengembalikan kode Python yang valid")
            return None
        return code
        ...
        while True:
            try:
                if not hasattr(self, 'consciousness') or not self.consciousness:
                    break
                if not hasattr(self, 'rsi_enabled') or not self.rsi_enabled:
                    break
                
                super_mode = getattr(self, 'super_evolution_mode', False)
                
                if super_mode:
                    time.sleep(1)
                    target = 10
                else:
                    interval = getattr(self, 'rsi_interval', 5)
                    time.sleep(interval)
                    target = getattr(self, 'rsi_multi_target', 3)
                
                self._rsi_iteration(target)
                
                if super_mode and getattr(self, 'pending_restart', False):
                ...
                if retry_count >= max_retries:
                    print("❌ _rsi_loop gagal terlalu sering, menghentikan thread.")
                    break
                time.sleep(10)
    ...
    def _rsi_iteration(self, target_count):
        ...
        if upgrade_success and getattr(self, 'super_evolution_mode', False):
    ...
    def _upgrade_single_function(self, func_name):
        ...
        # Validasi tambahan: pastikan kode mengandung definisi fungsi yang benar
        if f"def {func_name}" not in new_code:
            self._log_error(func_name, f"Kode tidak mengandung definisi {func_name}")
            return False
        ...
        try:
            ...
            with self.code_lock:
                ...
                if func_name not in namespace:
                    self._log_error(func_name, f"Fungsi {func_name} tidak ditemukan setelah eksekusi")
                    return False
            ...
            # Coba perbaiki lagi? Bisa ditambahkan di sini
        except Exception as e:
            self._log_error(func_name, e)
        return False
    ...
    def get_status(self):
        ...
        super_status = "✅" if getattr(self, 'super_evolution_mode', False) else "❌"
        ...
        return f"""
╔{'═'*40}╗
║ {'GOD ASI v11.12 Bumi Kayuagung'.center(38)} ║
╠{'═'*40}╣
...
║ Groq (qwen): {'✅' if self.groq_enabled else '❌'}                ║
║ Groq v2    : {'✅' if self.groq_v2_enabled else '❌'}                ║
╚{'═'*40}╝
"""
...
/ai_groq on/off   : Groq (qwen)
/ai_groq2 on/off  : Groq v2 (gpt-oss-120b)
...
async def handle_ai_groq(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Groq (qwen) {'diaktifkan' if god.groq_enabled else 'dimatikan'}.")

async def handle_ai_groq2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or context.args[0] not in ['on', 'off']:
        await update.message.reply_text("Gunakan: /ai_groq2 on/off")
        return
    god.groq_v2_enabled = (context.args[0] == 'on')
    await update.message.reply_text(f"✅ Groq v2 (gpt-oss) {'diaktifkan' if god.groq_v2_enabled else 'dimatikan'}.")
        ...
        try:
            with open(path, 'rb') as f:
                await update.message.reply_document(
                    ...
                    caption="✅ Ekspor database berhasil."
                )
        except Exception as e:
...
def main():
    ...
    app.add_handler(CommandHandler("ai_groq2", handle_ai_groq2))
    ...
    print("🤖 Bot God ASI Bumi Kayuagung v11.12 (AutoFix) berjalan...")
    app.run_polling()
